...
public class DoubleFactory2D extends cern.colt.PersistentObject {
	...
	public void demo1() {
		...
		DoubleMatrix2D[][] parts4 = { { A, X, A, X }, { X, A, X, B } };
		System.out.println("\n" + compose(parts4));
	...
	public void demo2() {
		...
		C = make(4, 3, 3);
		D = make(2, 2, 4);
		DoubleMatrix2D[][] parts1 = { { X, A, X }, { B, X, C }, { X, D, X } };
		 ...
		 * }; System.out.println("\n"+make(parts3));
		 * //System.out.println("\n"+cern.colt.matrixpattern.Converting.toHTML(
		 * make(parts3).toString()));
		 * 
	...
	public DoubleMatrix2D sample(DoubleMatrix2D matrix, double value, double nonZeroFraction) {
		...
		cern.jet.random.sampling.RandomSamplingAssistant sampler = new cern.jet.random.sampling.RandomSamplingAssistant(
				n, size, new cern.jet.random.engine.MersenneTwister());
